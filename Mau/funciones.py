...
def calcularOrdenNumero(a,b,c):
    """
    Funcionalidad:
    Ordena de mayor a menor los tres vzlores solicitos al usuario
    Entrada:
    -a, b, c(int):  Recibe 3 numeros enteros al usuario y los ordena
    de mayor a menor
    Salida:
    :Devolver los tres números ordenados
    """
    if a > b:
        if a > c:
            if b > c:
...
def gestionParticipantes(participantesCR, hombres, mujeres, mujeresConOro, hombresJovenesConMedallas):
    """
    Funcionalidad:
    Calcula, participantes de Costa Rica, hombres, hombres menores a 25 con mas de dos medallas, mujeres, y mujeres con almenos 1 medalla, según los datos solicitados
    Entrada:
    pais(int): Del 1 a 8 pide al usuario seleccionar entre los paises.
    sexo(int): Pide al usuario seleccionar entre 1 y 2, 1 es hombre, 2 es mujer.
    edad(int): Pide la edad del usuario.
    medallas(int): Pide la cantidad de medallas que lleva el usuario a su país.
    Salida:
    Participantes de Costa Rica, hombres, mujeres, hombres menores a 25 con 2 o más medallas, mujeres con almenos 1 medalla.
    """
     while True:
        try:
            if pais < 1 or pais > 8:
